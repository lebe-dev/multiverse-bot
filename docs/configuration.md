...
Отправляются только события об ошибках (стектрейсы): каждый `log.Error` уходит в Sentry, паники в горутинах перехватываются, после чего процесс падает как обычно. Performance tracing и профилирование выключены.

| Переменная | Требуется | По умолчанию | Описание |
|---|---|---|---|
| `SENTRY_DSN` | Нет | — | DSN проекта Sentry; пусто — отчётность отключена |
| `SENTRY_ENVIRONMENT` | Нет | — | Тег окружения для событий (например, `production`) |
